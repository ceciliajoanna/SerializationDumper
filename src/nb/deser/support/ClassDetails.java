...
public class ClassDetails {
    ...
    public String toString() {
        ...
        sb.append(String.format("%s\"__CLASS_TYPE__\":\"%s\"%s\n",
                CURRENT_INDENT + INDENT_SPACE,
                getClassName(),
                _fieldDescriptions.size() > 0 ? "," : ""
        ));

        for (int i = 0; i < _fieldDescriptions.size(); i++) {
            ClassField f = _fieldDescriptions.get(i);
            Object fieldValue = f.getValue();
            if (fieldValue == null) continue;
            if (fieldValue.getClass().isArray()) {
                Object[] arrayValues = (Object[]) fieldValue;
                int currentIndex = 0;

				for(int j=0;j<arrayValues.length;j++){
					if(arrayValues[j] instanceof ClassDataDesc) arrayValues[j] = ((ClassDataDesc) arrayValues[j]).getClassDetails(0);
				}
				sb.append(String.format("%s\"%s %s[%d]\":%s%s\n",
                        ...
                        i < _fieldDescriptions.size() - 1? ",":""
				));



            } else if (fieldValue instanceof ClassDataDesc) {
                ClassDataDesc cdd = (ClassDataDesc) fieldValue;
                if (fieldValue != null && cdd.getClassCount() > 0) {
                    ...
                    CURRENT_INDENT += INDENT_SPACE;
                    sb.append(cdd.getClassDetails(0).toString());

                    CURRENT_INDENT = previousIndent;
                    if (i < _fieldDescriptions.size() - 1)
                        sb.append(",\n");
                }
        ...
        sb.append(String.format("%s}", CURRENT_INDENT)).append("\n");
        return sb.toString();
    }
}
